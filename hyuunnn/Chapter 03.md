...
확장 함수는 정적 메서드와 같은 특징을 가지며, 클래스의 일부가 아닌 클래스 밖에 선언되기 때문에 하위 클래스에서 오버라이드할 수 없다.

확장 함수를 호출할 때 수신 객체로 지정한 **정적인 타입**에 의해 확장 함수가 결정되며, 변수가 가지고 있는 **동적인 객체 타입**으로 결정되지 않는다.
